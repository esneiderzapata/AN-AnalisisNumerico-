...
def biseccion(request):
	...
	if(f(xs)==0):
		...
		mensaje= 'NO HAY UNA RAIZ EN EL INTERVALO'

	if iterActual>=_iter:
		mensaje= 'El sistema no pudo hallar una raiz con la tolerancia indicada. '

	#print ('x',iterActual, '=',float(puntoMedio),' es una buena aproximacion')
	return render(request, 'biseccion.html', {'iterActual':iterActual, 'puntoMedio':float(puntoMedio), 'tablaBiseccion':tablaBiseccion, 'exp':exp, 'mensaje':mensaje, 'error':f"{float(error):.6f}"})
...
def newtonint(request):
	...
	if request.method == 'POST':
		xi = np.array(descifradorMatrices(str(request.POST.get('txtX'))))
		fi = np.array(descifradorMatrices(str(request.POST.get('txtY'))))
	...
	# expresión del polinomio con sppy
	x = sp.Symbol('x')
	polinomio = fi[0]
